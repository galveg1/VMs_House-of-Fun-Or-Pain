...
Goal: To report Lux sensor TSL2561 data to IOT website 
        Thingspeak
Steps: 

1- Declared a char array variable for pusing data unto Thing
speak through an ESP8255-01 module

2- Set clock to 40MHz 

3- Decalred variables for lux, lux averge and index

4- Configure UART, I2C, TSL2561  and Enable Hibernate

5- Print AT commands to UART for setting up the BAUD rate
    , Connection and mode of ESP8266-01

6- A for loop was used inside the main while loop to 
    collect and sum 20 data point from the sensor
    then it was averaged and pushed unto Thingspeak

**********************************************************
Chip type               : ARM TM4C123GH6PM
Program type            : Firmware
Core Clock frequency    : 40.000000 MHz
*******************************************************/
...
//Configures the UART to run at 19200 baud rate
{
    ...
    UARTStdioConfig(1, 19200, 16000000);   //enables UARTstdio baud rate, clock, and which UART to use
}
...
void main (void)
{
    ...
    //UARTprintf ("AT+RST\r\n");
    //SysCtlDelay (9000000); //CC66YOVY8SJNOAZ4
    ...
    while(1)
    {
        ...
        UARTprintf("AT+CIPSEND=%i\r\n", 47);//strlen(HTTP_POST));   //command the ESP8266 to allow sending of information
        SysCtlDelay (10000000);
        UARTprintf("GET /update?key=EMNPZFS0EFAEWB2V&field1=%d\n\r", luxAvg);
        UARTprintf("\n\r");
        ...
        //UARTprintf("AT+CIPSEND=%i\r\n", 47);//strlen(HTTP_POST));   //command the ESP8266 to allow sending of information
        //SysCtlDelay (10000000);
        //UARTprintf("GET /update?key=EMNPZFS0EFAEWB2V&field2=%d\n\r", lux);
        SysCtlDelay(34565555); //Delay for thinkgspeak
